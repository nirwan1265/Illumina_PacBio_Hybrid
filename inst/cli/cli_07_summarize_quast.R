main_07_summarize_quast <- function(args = commandArgs(trailingOnly = TRUE)) {
...
rows <- lapply(reports, function(p) {
  d <- read_quast(p)
  getv <- function(key) {
    if (key %in% names(d)) as.numeric(d[[key]]) else NA_real_
  }
  ...
  data.frame(
    ...
    total_length = getv("Total length"),
    n_contigs = getv("# contigs"),
    n50 = getv("N50"),
    largest_contig = getv("Largest contig"),
    gc = getv("GC (%)"),
    genome_fraction = getv("Genome fraction (%)"),
    mismatches_per_100kbp = getv("# mismatches per 100 kbp"),
    indels_per_100kbp = getv("# indels per 100 kbp"),
    misassemblies = getv("# misassemblies"),
